...
document.addEventListener("DOMContentLoaded", function () {
  ...
  parentMenus.forEach((parentMenu) => {
    ...
    // Add click event listener to the parentMenu
    parentMenu.addEventListener("click", function (event) {
      // Check if the click target is inside the submenu
      if (submenu.contains(event.target)) {
        return; // Do nothing if click is inside submenu
      }

      // Toggle the 'open' class on submenu
      if (submenu) {
        submenu.classList.toggle("open");
      }
    });
